...
const Assistant: React.FC<AssistantProps> = ({ activeProduct }) => {
  ...
  // Helper to convert image URL to Base64 for multimodal input
  const urlToBase64 = async (url: string): Promise<string | undefined> => {
    try {
        ...
        console.warn("Could not fetch image for AI context (likely CORS):", e);
        return undefined;
    }
  };
  ...
  const handleSend = async () => {
    ...
    try {
      // If there is an active product, try to fetch its image to let the AI "see" it
      let imageBase64: string | undefined = undefined;
      if (activeProduct) {
          imageBase64 = await urlToBase64(activeProduct.imageUrl);
      }

      const history = messages.map(m => ({ role: m.role, text: m.text }));
      const response = await sendMessageToGemini(history, userMsg.text, activeProduct, imageBase64);
        ...
        <div className="bg-[#F5F2EB] rounded-none shadow-2xl shadow-[#2C2A26]/10 w-[90vw] sm:w-[380px] h-[550px] mb-6 flex flex-col overflow-hidden border border-[#D6D1C7] animate-slide-up-fade">
          ...
          <div className="bg-[#EBE7DE] p-5 border-b border-[#D6D1C7] flex justify-between items-center">
            <div className="flex flex-col">
                ...
                {activeProduct && (
                    <span className="text-[10px] uppercase tracking-widest text-[#A8A29E] ml-5 mt-1">
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-8 bg-[#F5F2EB]" ref={scrollRef}>
            {messages.map((msg, idx) => (
              <div key={idx} className={`flex flex-col ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
                ...
                >
                  {msg.image && (
                      <div className="mb-3 w-full aspect-square overflow-hidden bg-[#F5F2EB]">
                          <img 
